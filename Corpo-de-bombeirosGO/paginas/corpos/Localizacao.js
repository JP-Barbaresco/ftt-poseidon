...
import { formLocalizacao, clearFormLocalizacao } from "../../actions/index";
...
export default function Localizacao({ navigation }) {
  ...
  async function buscaDeCorpos() {
    // let request = await fetch("http://192.168.100.25:8081/vitimas", {
    //   method: "POST",
    //   headers: {
    ...
    store.dispatch(clearFormLocalizacao);
    let form = {
      ultimoAvistamento: ultimoAvistamento,
    };
    store.dispatch(formLocalizacao(form));

    let toSend = {
      RAI: store.getState().equipe[0].RAI,
      nome_vitima: store.getState().vitimas[0].nome_vitima,
      ultimo_avistamento: store.getState().localizacao[0].ultimoAvistamento,
    };
          ...
          <ScrollView>
            <View style={globalStyles.cardClear}>
              ...
              <View style={globalStyles.pikerBotton}>
                <Picker
                  style={globalStyles.piker}
                  tipoLocal={tipoLocal}
                  onValueChange={(itemValue, itemIndex) =>
                    setTipoLocal(itemValue)
                  }
                >
              ...
              <Text style={globalStyles.titulo}>
                Tempo de Espera até Chegada do IML *
              </Text>
              <TextInput
                style={globalStyles.inputTypeB}
